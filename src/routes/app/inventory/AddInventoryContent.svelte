<script lang="ts">
  ...
  function addProduct(){
    disableSubmit = true
    fetch(
            "https://kori.azurewebsites.net/product/v1/77b5028d-5082-4dab-bdba-3fdc3fa35509",
            {
                method: "POST",
                headers: {
...
<div class="grid grid-cols-2 gap-x-12 gap-y-8 justify-between mb-8">
    ...
    <TextInput bind:value={price} label="Selling Price" placeholder="Enter Selling Price" type="number"/>
    <TextInput bind:value={reorder_level} label="Reorder level" placeholder="Enter Reorder level" type="number"/>
    <TextInput bind:value={measurement_unit} label="Measurement unit" placeholder="Enter Measurement unit" />
</div>
